    ...
    while (<BLASTIN>)                                           
    {
       if (substr($_, 0, 1) eq '#')                             #Ignore the comment lines starting with '#' in the BLAST files
        ...
        my $QGFF =                                      #Query features 
            $QryID."\t".
            $Prog."\t".    
#            $Prog."_HIT_$SubID\t".           # Feature (removed due to GFF3 validation conflicts)
            "match_part\t".           # Feature (SO definition: A part of a match, for example an hsp from blast is a match_part)
            $GQstart."\t".          # Start
            $GQend."\t".            # End
            $BitScore."\t".        # Score
            $QStrand."\t".          # Strand
...
#            "Name=$Prog"."_HIT_".$SubID.";ID=$QryID|$SubID|$GQstart|$GQend;Parent=$QryID" .                 # Attribute
        ...
        my $SGFF =                                      #Subject features
            $SubID."\t".
            $Prog."\t".    
#            $Prog."_HIT_$QryID\t".           # Feature (removed due to GFF3 validation conflicts)
            "match_part\t".           # Feature (SO definition: A part of a match, for example an hsp from blast is a match_part)
            $GSstart."\t".          # Start
            $GSend."\t".            # End
            $BitScore."\t".        # Score
